...
类型：提示词 / 高
候选：75_提示词库/...
建议：确认是否转正；不确定就继续核验或调整分类
可回复：1 确认转正 / 1 继续核验 / 1 调整分类：资料库 / 1 跳过
```
...
需要特别区分：生成脚本输出不等于前台消息已经触达用户。OpenClaw 必须实际把飞书精选链接或待确认决策卡发送出去；收到回复后必须写入 `85_运行记录/前台反馈队列.jsonl`。如果后台总览发现“已推送未反馈很多，但反馈队列为空”，应视为前台触达需要观察，而不是直接认为用户没处理。
