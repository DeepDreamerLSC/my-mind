...
- 转正门禁：候选文件必须带 `转正门禁` 字段和正文检查清单。用户或 OpenClaw 说“转为长期知识”时，只代表确认意图，Codex 后台仍要检查来源、解析、术语、中文表达、可复用结论和知识关联。
...
- 不把飞书当源库。
- 不直接改写已确认的长期知识正文。
- 不因为 OpenClaw 转述“转为长期知识”就绕过转正门禁；门禁未通过时，只能保留候选并给出最小补充问题。
