...
- When explicitly requested with `--extract-content`, extract audio from a public media URL with `ffmpeg`, transcribe with an available backend, then write only a short content summary, key points, timeline excerpt, and transcript excerpt. Do not write the full transcript by default.
