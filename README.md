...
当你一心扑进项目、容易陷入自己的局部判断时，Codex 可以定期生成决策审视和代码质量审视。前者挑战项目假设、偏航风险和机会成本；后者检查 Codex 产出代码的测试缺口、过度工程、配置风险和提交边界。
...
决策审视是项目的外部视角，不改项目文件，只指出隐含假设、反方意见、偏航风险、机会成本和下个最小可逆动作。
...
- `85_运行记录/决策审视-<project>-*.md`
- `85_运行记录/后台总览/决策审视-<project>.md`
- `85_运行记录/后台总览/决策审视.md`

如果审视结果是高风险，`advice-analysis` 会把它转成行动建议；必要时由 OpenClaw 只推一个短问题给你确认，不转发完整报告。

### 代码质量审视

代码质量审视用于检查 Codex 产出的代码是否可靠，重点看行为回归、测试缺口、AI 代码味道、配置/凭证风险、过度工程和提交边界。
...
- `85_运行记录/代码质量审视-<project>-*.md`
- `85_运行记录/后台总览/代码质量审视-<project>.md`
- `85_运行记录/后台总览/代码质量审视.md`

黄色或红色质量等级会进入行动建议和飞书驾驶舱。它不代表自动修复；下一步通常是补验证、拆批提交或进入正式 code review。
...
- `my-mind 后台总控日更`：每天 07:10 执行一次，串联反馈消费、前沿情报入箱、解析质量修复、收件箱分拣、前台推送、飞书精选发布、运行产物治理、后台总览、行动建议和飞书仪表盘同步。它会读取最新决策审视和代码质量审视，并使用 `gpt-5.4` / `xhigh` 推理强度。最终只输出 5 行以内摘要。
- `项目组合每日巡检`：每天 20:30 分别生成 `my-mind` 和 `edu-agent` 项目进展候选报告，再生成决策审视和代码质量审视，刷新行动建议。它读取 git、运行记录和项目文件，并输出 `项目判断`，但不自动 commit，也不自动 `--apply`。使用 `gpt-5.4` / `xhigh` 推理强度，最终只输出 5 行以内摘要。
...
- 定期审视项目决策和 Codex 代码质量，提醒偏航、测试缺口、过度工程和提交风险。
